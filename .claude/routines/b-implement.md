...
5. **実装ループ**（上記ルールに従う）:
   ...
   l. Draft PRをReadyに変換して本文を更新する（typecheckが通っていればCIを待たずにReady化してよい）:
      ...
      **全PR共通ルール**: 全てのPR本文に `## 🎮 このPRで遊べるようになるゲーム` セクションを必ず含めること。
      新しいゲームが追加されない場合は「なし（バグ修正・変更のみ）」と記載する。
      > ⚠️ **ヘッダ文字列は必ず `## 🎮 このPRで遊べるようになるゲーム` を使うこと。`## 🎮 このPRで改善されること` など別の文言は一切禁止。Issueの種別によらずヘッダは固定。**
